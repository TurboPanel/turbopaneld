...
before replacing the source tree and restarts it after provisioning (unless
`--no-start`, used by the in-process UI update path, which leaves the unit
running and lets the daemon restart itself).
