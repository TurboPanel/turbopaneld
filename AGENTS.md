...
| **Command execution logs** | `src/logs/` | Streamed command transcripts: redaction deny-set, `<stateDir>/spool/execution-logs/` spool, batched upload to `POST /api/daemon/v1/commands/:commandId/log`, orphan sweep. Control-plane side: `../turbopanel/src/lib/execution-logs/AGENTS.md`; capture details in `src/deploy/AGENTS.md` (Streamed transcript capture) |
| **Container logs (collector)** | `src/logs/container-collector.ts` | Per-container `docker container logs --follow --timestamps` tails. `docker ps` observes **liveness only** (filtered on Compose's own `com.docker.compose.project`); environment/service identity comes from the local `deployment.json` (`serviceIds` map), never from a `com.turbopanel.*` label that could have drifted. Redacted through the **process-wide** deny-set (`sharedSecretRedactor`, fed by every decrypt whether or not a collector is running), ring-buffered (drop-oldest, never spooled to disk), batched to `POST /api/daemon/v1/logs/containers`. Survives reconnects and container recreation: batches are held while the socket is down, and tails re-attach from process-wide per-container / per-environment cursors instead of `--tail 0`. Started/stopped by the `presence-ack` `containerLogsEnabled` flag — **no new command type**. Control-plane side: `../turbopanel/src/lib/container-logs/AGENTS.md` |
