...
- Logs are written to both journald and `/opt/turbopanel/platform/daemon/logs/{daemon.log,daemon.err.log}` when running under systemd. The `daemon-logs` role creates the directory and pre-touches the log files; no `tmpfiles.d` or logrotate entries are needed since the directory is inside the persistent checkout (gitignored in development). The official installer runs it via `daemon-launch`, and `initOrchestration()` re-runs `daemon-logs-setup.yml` on every daemon start so existing daemons pick it up without a full reinstall.
