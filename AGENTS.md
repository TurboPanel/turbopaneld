...
- **Shared orchestration runtime.** uv, Python, and Ansible are installed into
  ...
  `scripts/bootstrap-orchestration.ts`. The Deno orchestration functions
...
- `scripts/bootstrap-orchestration.ts` — thin Deno entry that runs `ensureUv` →
  `ensurePython` → `bootstrapOrchestrationRuntime`; used by the console and the
  CDN installer in place of the removed `bootstrap-orchestration.sh`.
