...
- **`dist/channels.json`** is committed to the repo and is the source of truth
  for the root catalog at `https://dl.trbp.nl/channels.json`. It is **not**
  regenerated by trunk builds — update it manually when new apps or channels are
  added, then re-publish via **`publish-channels-catalog`**.
- Run the **`publish-channels-catalog`** GitHub Actions workflow **manually**
  (via `workflow_dispatch`) after adding a new app or channel to
  `dist/channels.json`.
- Do **not** run it on every trunk push — the catalog does not change
  per-build.
- The **`publish-daemon-trunk`** workflow triggers on every push to **`trunk`**.
  ...
  immutable per-build artifacts to `/daemon/trunk/<buildId>/`, overwrites
  `/daemon/trunk/channel.json` with the latest manifest, and cleans up old build
  directories on the FTP server (keeping the two newest). Cleanup failure is
  tolerated and does not fail the publish job.
- ⚠️ Three GitHub repository secrets must be created before either publish
