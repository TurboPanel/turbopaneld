...
- **Presence:** after a successful WS connection the daemon sends `{ type: 'hello', at, agent }` once, then `{ type: 'heartbeat', at }` only when idle (no other WS traffic for 60 s). The instance records inbound activity via `recordInbound` with coalesced cell writes; there is no `heartbeat-ack`. Heartbeat is idle-only and debounced/throttled identically in both runtimes (≤ once/60 s for `lastSeenAt`). Status is projected to Postgres on connect/disconnect/missed-heartbeat/lease-expiration; UI/API status reads come from Postgres, never the cell. Workers (DO) and self-hosted (Redis) maintain daemon-cell behavior parity (implementation detail only) — see the instance repo `AGENTS.md` Daemon Cell section for cost/parity rules. No monitoring data is forwarded over the WS. The `Sentinel` continues to collect host/Docker state in-memory; a daemon-side SQLite monitoring store will consume it in a future effort.
...
- **Public URL apply**: a `public-urls-update` message (sent by the instance
  ...
  `TURBOPANEL_PUBLIC_URLS=<comma-joined>` into
  `/opt/turbopanel/platform/config/instance/runtime.env` (protected config — never
  the instance git checkout), then runs the
