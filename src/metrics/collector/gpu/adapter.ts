...
/**
 * Partial, per-field GPU reading. A field left unset (not present in the
 * object) or explicitly `null` both mean "this adapter has no reading for
 * this field this tick" — `gpu/index.ts`'s orchestrator merge resolves each
 * field independently along vendor adapter precedence, so either form
