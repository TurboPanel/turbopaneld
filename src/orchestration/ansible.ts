...
async function runLocalPlaybook(playbook: string, extraArgs: string[] = []): Promise<void> {
  await runOrThrow(
    ANSIBLE_PLAYBOOK_BIN,
...
export async function ensureGalaxyRoles(): Promise<void> {
  ...
  const galaxyRequirements = `${ORCHESTRATION_DIR}/requirements.yml`
  const rolesDir = `${ORCHESTRATION_DIR}/roles`
  const collectionsDir = `${ORCHESTRATION_DIR}/runtime/galaxy-collections`

  console.log(`[orchestration] installing galaxy roles from ${galaxyRequirements}`)
  await runOrThrow(
    galaxyBin,
    ['role', 'install', '-r', galaxyRequirements, '-p', rolesDir],
    { stream: true },
  )
  console.log('[orchestration] galaxy roles ready')

  console.log(`[orchestration] installing galaxy collections from ${galaxyRequirements}`)
  await runOrThrow(
    galaxyBin,
    [
      'collection',
...
export async function runLocalhostTest(): Promise<void> {
  ...
  await runLocalPlaybook(LOCALHOST_PLAYBOOK)
 ...
 */
export async function runDaemonConverge(): Promise<void> {
  const args = devInstanceExtraArgs()
  console.log('[orchestration] running daemon-converge playbook')
  await runLocalPlaybook(DAEMON_CONVERGE_PLAYBOOK, args)
...
export async function runSocketDirsSetup(): Promise<void> {
  ...
  await runLocalPlaybook(SOCKET_DIRS_PLAYBOOK)
...
export async function runDaemonLogsSetup(): Promise<void> {
  ...
  await runLocalPlaybook(DAEMON_LOGS_PLAYBOOK)
...
export async function runInstanceDevInstall(): Promise<void> {
  ...
  await runLocalPlaybook(INSTANCE_DEV_INSTALL_PLAYBOOK, args)
...
export async function runBuildToggle(opts: {
  ...
  await runLocalPlaybook(BUILD_TOGGLE_PLAYBOOK, args)
...
export async function runDockerSetup(): Promise<void> {
  ...
  await runLocalPlaybook(DOCKER_PLAYBOOK, devInstanceExtraArgs())
...
export async function runPostgresSetup(): Promise<void> {
  ...
  await runLocalPlaybook(POSTGRES_PLAYBOOK)
...
export async function runRedisSetup(): Promise<void> {
  ...
  await runLocalPlaybook(REDIS_PLAYBOOK)
...
export async function runRabbitmqSetup(): Promise<void> {
  ...
  await runLocalPlaybook(RABBITMQ_PLAYBOOK)
