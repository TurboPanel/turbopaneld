 ...
 * `src/contracts/commands/schemas.ts` module. Keep in sync when instance
 * command shapes change. These files are not byte-twins — instance schemas
 ...
 * `src/contracts/commands/schemas.ts`. `name` matches a `dockerExternalNetworks`
 ...
 *
 * Kept local rather than imported from `src/util/optional-fields.ts` for the same
 ...
 * `src/contracts/commands/schemas.ts` (no spaces) so a name the instance rejects can
 ...
 * instance `src/contracts/commands/schemas.ts`. Narrower than
 ...
 * (`MANAGED_ENGINE_RELEASES` in `src/features/managed/releases.ts`, surfaced as
...
/** Must stay in sync with instance `src/features/managed/settings.ts`. */
 ...
 * Must stay in sync with instance `src/contracts/commands/schemas.ts`.
 */
...
/** Must stay in sync with instance `POSTGRES_RESERVED_ENV_KEYS` in `src/features/managed/settings.ts`. */
 ...
 * `MANAGED_RESERVED_ENV_KEYS_BY_ENGINE` in instance `src/features/managed/settings.ts`.
 */
 ...
 * `parseManagedDockerOptions` in `src/features/managed/settings.ts`. `extraEnv`
...
/** Must stay in sync with instance `src/features/managed/types.ts`. */
...
/** Mirrors the daemon `SAFE_MANAGED_ID_RE` (`src/managed/engine-paths.ts`) — backupId becomes a filename. */
