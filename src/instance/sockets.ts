...
export function resolveInstanceSocket(
  ...
  const override = resolved.TURBOPANEL_SOCKET?.trim();
  if (override) return override;

  const layout = resolveLayout(resolved);
  const dir = resolved.TURBOPANEL_SOCKET_DIR?.trim() || layout.runDir;
  return `${stripTrailingSlashes(dir)}/${INSTANCE_SOCKET}`;
}
...
export function resolveInstanceConfig(
  ...
  return { kind: "socket", socketPath: resolveInstanceSocket(resolved) };
}
...
export function resolveInstanceCaPath(
  ...
  const resolved = envOrProcess(env);
  const fromEnv = resolved.TURBOPANEL_INSTANCE_CA?.trim();
  if (fromEnv && fileExistsSync(fromEnv)) return fromEnv;
  const canonicalPath = resolveLayout(resolved).instanceCaPath;
