...
test("legacySweepLegacyManagedTraefikIngress downs stale projects and removes claim tree", async () => {
  const { createTempLayout } = await import("../testing/temp-layout.ts");
  const { resolveLayout } = await import("../paths/layout.ts");
  const { legacySweepLegacyManagedTraefikIngress } = await import(
    "./proxysql.ts"
  );
  const fixture = await createTempLayout();
  try {
    ...
    const ingressDir = join(layout.stateDir, "managed", managedId, "ingress");
    await Deno.mkdir(ingressDir, { recursive: true });
    await Deno.writeTextFile(
    ...
    await Deno.writeTextFile(join(claimDir, `${managedId}.json`), "{}");

    const calls: string[][] = [];
    await legacySweepLegacyManagedTraefikIngress(layout, async (args) => {
      calls.push([...args]);
      return { success: true, stdout: "", stderr: "", code: 0 };
    });

    assertEquals(calls.length, 1);
    assertEquals(calls[0]?.[0], "compose");
    assertEquals(calls[0]?.[2], `turbopanel-managed-${managedId}-ingress`);

    let gone = false;
    try {
      await Deno.stat(ingressDir);
    } catch (err) {
      if (err instanceof Deno.errors.NotFound) gone = true;
      else throw err;
    }
    ...
    let claimGone = false;
    try {
      await Deno.stat(claimDir);
    } catch (err) {
      if (err instanceof Deno.errors.NotFound) claimGone = true;
      else throw err;
    }
    assertEquals(claimGone, true);
  } finally {
    await fixture.cleanup();
  }
});
