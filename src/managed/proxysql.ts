...
/** Directory entries under `<stateDir>/managed/`, or `null` when absent. */
async function listManagedEntries(
  managedRoot: string,
): Promise<Deno.DirEntry[] | null> {
  const entries: Deno.DirEntry[] = [];
  try {
    for await (const entry of Deno.readDir(managedRoot)) {
      entries.push(entry);
    }
    return entries;
  } catch (err) {
...
/** Bring down one legacy per-service managed ingress compose project. */
async function downLegacyManagedIngressProject(
  managedId: string,
  ingressCompose: string,
  run: RunDockerFn,
): Promise<void> {
  const project = `turbopanel-managed-${managedId}-ingress`;
  ...
  if (!down.success) {
    logInfo(
      "managed",
      `legacy managed ingress down soft-failed project=${project}: ${
...
/** Remove one legacy managed ingress directory, tolerating prior removal. */
async function removeLegacyManagedIngressDir(
  managedRoot: string,
  managedId: string,
): Promise<void> {
  try {
    await Deno.remove(join(managedRoot, managedId, "ingress"), {
      recursive: true,
    });
  } catch (err) {
    if (!(err instanceof Deno.errors.NotFound)) {
      logInfo(
        "managed",
        `legacy managed ingress dir remove soft-failed managedId=${managedId}: ${
...
/** Sweep one managed-directory entry's legacy per-service ingress, if any. */
async function sweepLegacyManagedTraefikEntry(
  managedRoot: string,
  entry: Deno.DirEntry,
  run: RunDockerFn,
): Promise<void> {
  if (!entry.isDirectory || !SAFE_MANAGED_ID_RE.test(entry.name)) return;
  const ingressCompose = join(
    managedRoot,
    entry.name,
    "ingress",
    "docker-compose.yml",
  );
  if (!(await pathExists(ingressCompose))) return;
  await downLegacyManagedIngressProject(entry.name, ingressCompose, run);
  await removeLegacyManagedIngressDir(managedRoot, entry.name);
}

/** Remove the shared legacy managed-ingress claim directory, if present. */
async function removeLegacyManagedIngressClaimDir(
  layout: LayoutPaths,
): Promise<void> {
  const claimDir = join(layout.stateDir, "managed", "ingress");
  try {
    await Deno.remove(claimDir, { recursive: true });
  } catch (err) {
    if (!(err instanceof Deno.errors.NotFound)) {
      logInfo(
        "managed",
        `legacy managed ingress claim dir remove soft-failed: ${
          sanitizeForLog(err instanceof Error ? err.message : String(err))
        }`,
      );
    }
...
/**
 * Idempotently tear down legacy per-service managed Traefik projects left
 * from upgrades before ProxySQL ingress.
 */
export async function legacySweepLegacyManagedTraefikIngress(
  layout: LayoutPaths,
  run: RunDockerFn = defaultRunDocker,
): Promise<void> {
  const managedRoot = join(layout.stateDir, "managed");
  const managedEntries = await listManagedEntries(managedRoot);
  if (managedEntries === null) return;

  for (const entry of managedEntries) {
    await sweepLegacyManagedTraefikEntry(managedRoot, entry, run);
  }

  await removeLegacyManagedIngressClaimDir(layout);
}
