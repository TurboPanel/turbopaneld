...
/** Byte-identical order with instance `src/features/commands/types.ts` COMMAND_TYPES. */
...
test("managed.apply enforces the engine image allowlist", () => {
  ...
  // (`src/features/managed/releases.ts`, surfaced through
  // `src/features/managed/settings.ts`) so a forged/replayed command payload cannot
